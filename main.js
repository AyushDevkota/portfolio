...
const international = [
	"Participated in the International Leaders programme on invitation of the Foreign & Commonwealth Office British Government in London November 22-27, 2015",
...
const education = [
	...
	{
		bold: "St. Stephen's College, University of Delhi",
		place: "India.",
		paragraphs: [
	...
	{
		bold: "Lawrence School",
		place: "Sanawar (Shimla Hills), India.",
		paragraphs: ["All India Senior School Certificate Examination, May 1988"],
	},
	{
		bold: "St. Joseph's College (North Point)",
		place: "Darjeeling, India.",
		paragraphs: ["Indian Certificate of Secondary Education, February 1986"],
	},
];
...
const personal = [
	{
		key: "Name",
		value: "Udaya S.J.B Rana",
	},
	{
		key: "Date of Birth",
		value: "February 27, 1970",
	},
	{
		key: "Address",
		value: "Kupandole Hts., Lalitpur Ward – I, Kathmandu, Nepal",
	},
	{
		key: "Telephone",
		value: "5523608 / 5527871",
	},
	{
		key: "Mobile",
		value: "+977-9851061424",
	},
	{
		key: "Email",
		value: "nepaltdl@mos.com.np, contact@udayarana.com",
	},
];
...
// Add paragraphs inside biography section
const bioTitle = document.querySelector("#bio-title");
let htmlBio = "";
bioParagraphs.forEach((el) => {
	htmlBio += `<p class = "py-3 text-center sm:text-left">${el}</p>`;
});
bioTitle.insertAdjacentHTML("afterend", htmlBio);

// Accordion
const accordionElement = document.querySelector("#ul");
let htmlAcc = "";
accordionTitle.forEach((el, index) => {
	htmlAcc += `
	<li class="relative border-b border-gray-200 text-white">
		<button type="button" class="w-full px-8 py-6 text-left" @click="selected !== ${index} ? selected = ${index} : selected = null">
			<div class="flex items-center justify-between">
				<span class="text-black">${el.toUpperCase()}</span>
		...
		<div class="relative overflow-hidden transition-all max-h-0 duration-700" style="" x-ref="container${index}" x-bind:style="selected == ${index} ? 'max-height: ' + $refs.container${index}.scrollHeight + 'px' : ''">
			<div class="p-6 text-black" id="${el.replaceAll(" ", "")}">
			</div>
		</div>
	</li>`;
});
accordionElement.insertAdjacentHTML("afterend", htmlAcc);

function createList(id, arr) {
	const ele = document.querySelector(`#${id}`);
	let htmlList = "<ul class='list-disc pl-4'>";
	arr.forEach((item) => {
		htmlList += `<li class="pb-4">${item}</li>`;
	});
	htmlList += "</ul>";
	ele.insertAdjacentHTML("beforeend", htmlList);
}
createList("publications", publication);
createList("skills", skills);
createList("internationalseminarsandtrainings", international);
createList("travel", travel);
createList("politicalbackground", politicalBackground);

function createActivities(id, arr) {
	const act = document.querySelector(`#${id}`);
	let htmlAct = "<ul class='list-disc pl-4'>";
	arr.forEach((item) => {
		htmlAct += `<li class="font-bold pt-10">${item.title}</li>`;
		item.italics.forEach((content) => {
			htmlAct += `<p class="italic py-1">${content}</p>`;
		});
		htmlAct += `<p class="py-1">${item.text}</p>`;
	});
	htmlAct += "</ul>";
	act.insertAdjacentHTML("beforeend", htmlAct);
}
createActivities("activities", activities);
createActivities("workexperience", workExp);

function createEducation(arr) {
	const edu = document.querySelector("#education");
	let htmlEdu = "<ul class='list-disc pl-4'>";
	arr.forEach((item) => {
		htmlEdu += `<li class="pt-10"><span class="font-bold pt-10">${item.bold}</span>, <span>${item.place}</span></li>`;
		item.paragraphs.forEach((content) => {
			htmlEdu += `<p class="pb-1">${content}</p>`;
		});
	});
	htmlEdu += `</ul>`;
	edu.insertAdjacentHTML("beforeend", htmlEdu);
}
createEducation(education);

function createPersonal(arr) {
	const per = document.querySelector("#personal");
	let htmlPer = `<div class="bg-white shadow overflow-hidden sm:rounded-lg"><div class="border-t"><dl>`;
	arr.forEach((item) => {
		htmlPer += `
	...
	htmlPer += `</dl></div></div>`;
	per.insertAdjacentHTML("beforeend", htmlPer);
}
